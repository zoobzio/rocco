...
The engine's `WriteTimeout` caps how long a single response may take. Stream
handlers are automatically exempt: each stream clears its own write deadline
before sending, so a long-lived stream is never torn down at `WriteTimeout`.
You do not need to configure anything for streams to outlive it.

`WriteTimeout` still applies to regular (non-stream) handlers. Set the engine
timeouts with `WithTimeouts(read, write, idle)`:

```go
engine := rocco.NewEngine().
    WithTimeouts(15*time.Second, 30*time.Second, 120*time.Second)
```

The Go standard library has no per-route write timeout — `WriteTimeout` is a
single value for the whole server. rocco works around this for streams by
clearing the write deadline on the streaming connection; it does not change the
timeout for any other route. `ReadTimeout` and `IdleTimeout` apply to streams
normally.

Because a cleared write deadline means a stalled client (one that stops reading)
would block `Send` indefinitely, always watch `stream.Done()` and enforce
backpressure — see the practices below.

The initial request body of a POST/PUT stream is capped at 10MB by default.
Adjust it per handler with `WithMaxBodySize`; a value of `0` removes the limit.
